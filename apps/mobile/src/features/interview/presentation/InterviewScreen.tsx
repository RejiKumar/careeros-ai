...
export default function InterviewScreen() {
  ...
  async function handleSubmitAnswer() {
    if (activeQuestionId === null || answerDraft.trim() === "" || answerState.status === "loading") {
      return;
    }
    ...
    return (
      <AppBackground>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t("interview.modeSection")}</Text>
          ...
          {resumes.length > 0 && (
            <>
              <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t("interview.resumeContext")}</Text>
              <View style={styles.chipRow}>
                {resumes.map((r) => (
                  ...
                  >
                    <Text
                      style={[styles.chipText, { color: resumeId === r.id ? colors.onPrimary : colors.textPrimary }]}
                    >
                      {r.title}
                    </Text>
                  </Pressable>
          ...
          <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t("interview.targetJob")}</Text>
          <TextInput
            value={targetJob}
          ...
          <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t("interview.targetSkills")}</Text>
          <TextInput
            value={targetSkills}
          ...
          >
            {creating ? (
              <ActivityIndicator color={colors.onPrimary} accessibilityLabel={t("interview.startButton")} />
            ) : (
              <Text style={[styles.primaryButtonText, { color: colors.onPrimary }]}>{t("interview.startButton")}</Text>
            )}
          </Pressable>
        </ScrollView>
      </AppBackground>
  ...
  // Session view
  const activeQuestion = sessionDetail.status === "success"
    ...
    <AppBackground>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Pressable
          ...
          onPress={() => { setViewMode("setup"); setActiveSession(null); setActiveQuestionId(null); }}
          style={[styles.backLink, { backgroundColor: colors.surfaceRaised }]}
        >
          <Text style={[styles.backLinkText, { color: colors.textPrimary }]}>{t("interview.newSession")}</Text>
        </Pressable>

        <ScreenHeader eyebrow={t("interview.eyebrow")} title={`${activeSession?.mode ?? ""} session`} />
        ...
        {sessionDetail.status === "error" && (
          <View style={[styles.errorCard, { backgroundColor: colors.danger }]} accessibilityRole="alert">
            <Text style={[styles.errorText, { color: colors.onDanger }]}>{sessionDetail.message}</Text>
          </View>
        )}

        {sessionDetail.status === "success" && (
          <>
            <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{t("interview.questionsSection")}</Text>
              ...
              return (
                <Pressable
                  ...
                  style={[
                    styles.questionCard,
                    {
                      backgroundColor: activeQuestionId === q.id ? colors.primarySoft : colors.surface,
                      borderColor: activeQuestionId === q.id ? colors.primary : colors.border,
                    },
                  ]}
                >
                  <View style={styles.questionHeader}>
                    <Text style={[styles.questionNumber, { color: colors.primaryStrong }]}>{i + 1}</Text>
                    <Text style={[styles.questionFocus, { color: colors.textDisabled }]}>{q.focus}</Text>
                    {answered && <Text style={[styles.answeredBadge, { color: colors.success }]}>✓</Text>}
                  </View>
            ...
            {activeQuestion !== null && (
              <View style={[styles.answerSection, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.answerSectionTitle, { color: colors.textPrimary }]}>
                  {t("interview.answerTitle", { n: sessionDetail.questions.findIndex((q) => q.id === activeQuestionId) + 1 })}
                </Text>
                <TextInput
                  value={answerDraft}
                ...
                <Pressable
                  ...
                  style={({ pressed }) => [
                    ...
                    { backgroundColor: colors.primary },
                    pressed && styles.pressed,
                    (answerDraft.trim() === "" || answerState.status === "loading") && styles.disabled,
                  ]}
                >
                  {answerState.status === "loading" ? (
                    <ActivityIndicator color={colors.onPrimary} accessibilityLabel={t("common.loading")} />
                  ) : (
                    <Text style={[styles.primaryButtonText, { color: colors.onPrimary }]}>{t("interview.submitAnswer")}</Text>
                  )}
                </Pressable>

                {answerState.status === "error" && (
                  <View style={[styles.errorCard, { backgroundColor: colors.danger }]} accessibilityRole="alert">
                    <Text style={[styles.errorText, { color: colors.onDanger }]}>{answerState.message}</Text>
                  </View>
                )}

                {answerState.status === "success" && (
                  <View style={[styles.evalCard, { backgroundColor: colors.primarySoft, borderColor: colors.primary }]}>
                    <Text style={[styles.evalTitle, { color: colors.primaryStrong }]}>{t("interview.evaluation")}</Text>
                    {([
                      ...
                      { evalKey: "technical_correctness", i18nKey: "interview.dimensions.technicalCorrectness" },
                      { evalKey: "completeness", i18nKey: "interview.dimensions.completeness" },
                    ] as const).map(({ evalKey, i18nKey }) => (
                      <View key={evalKey} style={styles.evalRow}>
                        <Text style={[styles.evalLabel, { color: colors.textSecondary }]}>{t(i18nKey)}</Text>
                        <Text style={[styles.evalScore, { color: colors.primaryStrong }]}>
                          {answerState.evaluation[evalKey]}/10
                        </Text>
                      </View>
                    ))}
                    <Text style={[styles.evalFeedback, { color: colors.textPrimary }]}>{answerState.evaluation.feedback}</Text>
                    {answerState.evaluation.suggested_answer !== "" && (
                      <View style={styles.suggestedBlock}>
                        <Text style={[styles.suggestedLabel, { color: colors.textSecondary }]}>{t("interview.suggestedAnswer")}</Text>
                        <Text style={[styles.suggestedText, { color: colors.textPrimary }]}>{answerState.evaluation.suggested_answer}</Text>
                      </View>
                    )}
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  backLink: { borderRadius: 9999, paddingHorizontal: 14, paddingVertical: 8, alignSelf: "flex-start", marginTop: 16 },
  backLinkText: { fontSize: 13, fontWeight: "600" },
  eyebrow: { fontSize: 13, fontWeight: "700", letterSpacing: 1.2, textTransform: "uppercase", marginTop: 16 },
  ...
  answerInput: { borderWidth: 1, borderRadius: 12, padding: 14, fontSize: 15, minHeight: 100, textAlignVertical: "top" },
